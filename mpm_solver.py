            ...
            if self.grid_m[I] > 0:  # No need for epsilon here
                ...
                for d in ti.static(range(self.dim)):
                    ...
                    if ((grid_pos - ti.Vector([3.5, 11.5, 7.0])).norm() - 1.5**0.5 < 0):
                        self.grid_v[I] = [0, 0, 0]
                    if ((grid_pos - ti.Vector([11.3,8.6,6.5])).norm() - 0.75**0.5 < 0):
                        self.grid_v[I] = [0, 0, 0]
                    ...
                    x2 = max(abs(box_position_rotated[0]) - size2[0], 0.0)
                    y2 = max(abs(box_position_rotated[1]) - size2[1], 0.0)
                    z2 = max(abs(box_position_rotated[2]) - size2[2], 0.0)
                    q2 = ti.Vector([x2,y2,z2])
